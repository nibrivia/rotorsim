...
tput_plot <- function(dir, flow_fns) {
    experiments <<- tibble(filename = flow_fns) %>%
      mutate(exp_id     = seq_along(filename),
             base_fn    = strsplit(flow_fns, "ms.csv") %>% map_chr(1) %>% strsplit("skewed-") %>% map_chr(2) ,
             split = strsplit(base_fn, "-"),
             n_tor      = as.integer(map_chr(split, 1)),
             n_switches = strsplit(map_chr(split, 2), ":"),
             n_xpand    = as.integer(map_chr(n_switches, 2) %>% strsplit(., ",") %>% map(2)),
  ...
  tputs <<- experiments %>% 
    mutate(arrivals = map(paste0(dir, filename),
                           function(fn) {
                             read_csv(fn, col_types = "--iid-ddd") %>% 
                               filter(!is.na(fct)) %>% 
                               group_by(src) %>%
  ...
  tputs %>%
    ...
    geom_point() +
    geom_line() +
    geom_text(aes(label = max_time), hjust = "inward") +
    
    scale_x_percent() +
    
    theme_ipsum_rc() +
    theme(legend.position = c(.85, .85)) +
    labs(title = "ToR Throughput of skewed workload by load",
         subtitle = paste0(exp$n_tor, " tors ",
                           "for ", exp$duration, "ms. ",
                           exp$n_switches, " switches: ",
                           exp$n_cache, " cache, ",
...
dir <- "../data/"
flow_fns <- c(
"skewed-129-21:0,5-chen-0.875-1000ms.csv",
"skewed-129-21:0,5-chen-0.75-1000ms.csv",
"skewed-129-21:0,5-chen-0.625-1000ms.csv",
"skewed-129-21:0,5-chen-0.5-1000ms.csv",
"skewed-129-21:0,5-chen-0.375-1000ms.csv",
"skewed-129-21:0,5-chen-0.25-1000ms.csv",
"skewed-129-21:0,5-chen-0.125-1000ms.csv",
"skewed-129-21:8,5-chen-0.875-1000ms.csv",
"skewed-129-21:8,5-chen-0.75-1000ms.csv",
"skewed-129-21:8,5-chen-0.625-1000ms.csv",
"skewed-129-21:8,5-chen-0.5-1000ms.csv",
"skewed-129-21:8,5-chen-0.375-1000ms.csv",
"skewed-129-21:8,5-chen-0.25-1000ms.csv",
"skewed-129-21:8,5-chen-0.125-1000ms.csv"
)
tput_plot(dir, flow_fns)
```
