...
tput_plot <- function(dir, flow_fns) {
  tputs <<- experiments %>% 
    mutate(arrivals = map(filename,
                           function(fn) {
                             read_csv(fn, col_types = "--iid-ddd", skip = 2) %>% 
                               filter(!is.na(fct)) %>% 
                               group_by(src) %>%
                                 summarize(bits_sent  = sum(sent),
                                           max_t      = max(start+fct)) %>%
                                ungroup() %>% 
                               summarize(mean_sent = mean(bits_sent)/max(max_t/1e3),
                                         max_time  = max(max_t))
                               })) %>% 
    unnest(arrivals)
...
dir <- "../data/"
experiments <- tibble(
  filename = list.files(path = "../data/", pattern = ".*ms(.+-){4}.+.csv", full.names = TRUE)
) %>% 
  mutate(metadata = map(filename, read_csv, n_max = 1)) %>% 
  unnest(metadata)

tput_plot(experiments)
```
