...
dir <- "../simulator/"
flow_fns <- list.files(path = dir, pattern = "^108.*datamining.*-1000ms.csv")
...
fcts <- flows %>%
  filter(!is.na(fct)) %>% 
  #filter(load == .01) %>% 
  group_by(exp_id, load, size, tag, n_cache) %>% 
...
fcts %>% 
  ...
  ggplot(aes(x = size/8,
             y = fct*1e3,
             color = load_name,
             linetype = as_factor(n_cache),
             #shape = load_name,
             group = exp_id)) +
  geom_line(show.legend = T) +
  geom_point(size = 2, aes(color = tag)) +
