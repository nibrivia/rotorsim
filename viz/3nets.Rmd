...
```{r many-experiments, }
flow_fns <- list.files(path = "../simulator/", pattern = "drain.*10000ms.csv")
...
flows <- experiments %>% 
  mutate(arrivals = map(paste0("../simulator/", filename), read_csv, col_types = "iiidddd")) %>% 
  select(exp_id, arrivals) %>% 
  unnest(arrivals) %>% 
...
if (arrive_at_start) {
  end_times <- flows %>%
    ...
    group_by(exp_id) %>%
      ...
      ungroup() %>% 
    reshape2::melt(measure.vars = c("mean", "90%-ile"),
                   variable.name = "type",
                   value.name    = "fct") %>% 
  ...
  end_times %>% 
    ...
    geom_line() +
    #annotate("blank", x = c(0,1), y = c(0,10)) +
    geom_point(size = 4) +
    scale_x_percent() +
    theme_ipsum_rc() +
    theme(legend.position = c(.9, .4),
          legend.background = element_rect(fill = alpha("white", .7),
                                           size = 0)) +
    labs(title = "90%-ile drainage time per rotor-pair",
         subtitle = "129 ToRs for 10s, 32 switches, 0 expander",
         color = "# cache sw",
         shape = "# cache sw",
         linetype = NULL,
         x = "Actual load",
