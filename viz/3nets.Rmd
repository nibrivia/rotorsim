...
```{r many-experiments, }
flow_fns <- list.files(path = "../simulator/", pattern = "drain.*1000ms.csv")
...
flows <- experiments %>% 
  mutate(arrivals = map(paste0("../simulator/", filename), read_csv, col_types = "iddd")) %>% 
  select(exp_id, arrivals) %>% 
  unnest(arrivals)
...
if (arrive_at_start) {
  flows %>%
    ...
    group_by(exp_id) %>%
      summarize(fct = mean(end),
                fct_hi = quantile(end, .9)) %>%
      ungroup() %>% 
    ...
    geom_point(size = 5) +
    theme_ipsum_rc() +
    labs(title = "90-percentile drainage time per rotor-pair (ish)",
