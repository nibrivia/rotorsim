...
class Log:
    ...
    def set_fn(self, fn = "out", uuid = None, **params):
        ...
        param_names = [ "uuid",
                ...
                "workload", "arrive_at_start", "skewed", "load", "is_ml", "cache_policy", "valiant",
    ...
    def log_flow_done(self, flow):
        msg = "%d,%s,%d,%d,%.4f,%.4f,%d,%d,%.4f\n" % (
                flow.id, flow.tag, flow.src, flow.dst, flow.arrival, flow.end, flow.size_bits, flow.size_bits-flow.bits_left, flow.end - flow.arrival)
